...
# Get the project name from the repo's origin URL.
def getProject(repoPath):
    command = ['git', 'config', '--get', 'remote.origin.url']
    url = simpleShellExecute(command, repoPath)
    matches = re.search('^.+\.com/(\w+)/.+$', url)
...
class CopyStashCommand(sublime_plugin.TextCommand):
  ...
  def run(self, edit, gitEnabled=False, jiraLink=False):
    ...
    targetProject = getProject(paths[3])
